...
BlockOutput::BlockOutput(Windows::BaseWindow *window, QGraphicsItem *parent) :
    BaseBlock(::ItemType::BlockOutputType, window, parent)
{
    setSize(40, 40);
    setBaseName(QStringLiteral("Output"));
    ...
    getConnector(false, 0)->setVisible(false);
...
gpds::container BlockOutput::to_container() const {
    gpds::container root;
...
void BlockOutput::from_container(const gpds::container &container) {
    BaseBlock::from_container(*container.get_value<gpds::container *>("operation").value());
    _index = container.get_value<double>("index").value();
}

std::shared_ptr<QSchematic::Items::Item> BlockOutput::deepCopy() const {
    auto clone = std::make_shared<BlockOutput>(parentWindow(), parentItem());
    copyAttributes(*clone);

    return clone;
}

void BlockOutput::copyAttributes(BlockOutput &dest) const {
    BaseBlock::copyAttributes(dest);

    dest._index = _index;
}

Solver::BlockType BlockOutput::getSolverBlockType() const {
...
void BlockOutput::solveAlgebraic(const QVector<Solver::Signal> &in, QVector<Solver::Signal> &out, const QVector<Solver::Signal> &states) {
    out[0] = in[0];
}
